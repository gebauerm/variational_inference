...
sigma_est = np.random.normal(0, 1, k)
...
for iter in range(iterations):
    ...
    e_p_mu_k = sum([-mu_est[idk]**2 - sigma_est[idk] for idk in range(k)])
    ...
    e_log_mu_k_q = sum([(math.log(2*math.pi) + sigma_est[idk])/2 for idk in range(k)]) # here it might be one according to princton
    # compute elbo
